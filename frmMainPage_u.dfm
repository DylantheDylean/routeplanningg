object frmMainPage: TfrmMainPage
  Left = 0
  Top = 0
  Caption = 'Form4'
  ClientHeight = 731
  ClientWidth = 983
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OnShow = FormCreate
  TextHeight = 15
  object lblLocationName: TLabel
    Left = 24
    Top = 26
    Width = 81
    Height = 15
    Caption = 'Location Name'
  end
  object lblLatitude: TLabel
    Left = 25
    Top = 59
    Width = 43
    Height = 15
    Caption = 'Latitude'
  end
  object lblLongitude: TLabel
    Left = 24
    Top = 89
    Width = 54
    Height = 15
    Caption = 'Longitude'
  end
  object lblAddress: TLabel
    Left = 26
    Top = 121
    Width = 42
    Height = 15
    Caption = 'Address'
  end
  object pnlControls: TPanel
    Left = -15
    Top = 0
    Width = 312
    Height = 737
    TabOrder = 0
  end
  object pnlMap: TPanel
    Left = 303
    Top = 0
    Width = 682
    Height = 737
    TabOrder = 1
  end
  object grpLocationInput: TGroupBox
    Left = 8
    Top = 8
    Width = 264
    Height = 187
    Caption = 'Location Input'
    TabOrder = 2
  end
  object edtLocationName: TEdit
    Left = 136
    Top = 23
    Width = 121
    Height = 23
    TabOrder = 3
    Text = 'Edit1'
  end
  object edtLatitude: TEdit
    Left = 136
    Top = 56
    Width = 121
    Height = 23
    TabOrder = 4
    Text = 'Edit2'
  end
  object edtLongitude: TEdit
    Left = 136
    Top = 86
    Width = 121
    Height = 23
    TabOrder = 5
    Text = 'Edit3'
  end
  object edtAddress: TEdit
    Left = 136
    Top = 118
    Width = 121
    Height = 23
    TabOrder = 6
    Text = 'Edit4'
  end
  object grpMapOptions: TGroupBox
    Left = 8
    Top = 201
    Width = 264
    Height = 184
    Caption = 'Map Options'
    TabOrder = 7
  end
  object btnAddLocation: TButton
    Left = 24
    Top = 224
    Width = 115
    Height = 18
    Caption = 'Add Location'
    TabOrder = 8
    OnClick = btnAddLocationClick
  end
  object btnSetAsEnd: TButton
    Left = 25
    Top = 248
    Width = 114
    Height = 18
    Caption = 'Set As End'
    TabOrder = 9
    OnClick = btnSetAsEndClick
  end
  object btnClearRoute: TButton
    Left = 145
    Top = 272
    Width = 121
    Height = 18
    Caption = 'Clear Route'
    TabOrder = 10
    OnClick = btnClearRouteClick
  end
  object btnCalculateRoute: TButton
    Left = 26
    Top = 272
    Width = 114
    Height = 18
    Caption = 'Calculate Route'
    TabOrder = 11
    OnClick = btnCalculateRouteClick
  end
  object btnSetAsStart: TButton
    Left = 145
    Top = 224
    Width = 121
    Height = 18
    Caption = 'Set As Start'
    TabOrder = 12
    OnClick = btnSetAsStartClick
  end
  object btnRemoveLocation: TButton
    Left = 145
    Top = 248
    Width = 121
    Height = 18
    Caption = 'Remove Location'
    TabOrder = 13
    OnClick = btnRemoveLocationClick
  end
  object btnSaveMap: TButton
    Left = 147
    Top = 296
    Width = 121
    Height = 18
    Caption = 'Save Map'
    TabOrder = 14
    OnClick = btnSaveMapClick
  end
  object btnClearAll: TButton
    Left = 26
    Top = 296
    Width = 115
    Height = 18
    Caption = 'Clear All'
    TabOrder = 15
    OnClick = btnClearAllClick
  end
  object grpLocations: TGroupBox
    Left = 8
    Top = 391
    Width = 264
    Height = 154
    Caption = 'Locations'
    TabOrder = 16
  end
  object lstLocations: TListBox
    Left = 24
    Top = 414
    Width = 233
    Height = 97
    ItemHeight = 15
    TabOrder = 17
    OnClick = lstLocationsClick
  end
  object grpRoute: TGroupBox
    Left = 8
    Top = 568
    Width = 264
    Height = 159
    Caption = 'Route'
    TabOrder = 18
  end
  object memoRouteInfo: TMemo
    Left = 26
    Top = 592
    Width = 231
    Height = 121
    Lines.Strings = (
      'memoRouteInfo')
    ScrollBars = ssVertical
    TabOrder = 19
  end
  object chkShowGrid: TCheckBox
    Left = 26
    Top = 328
    Width = 97
    Height = 17
    Caption = 'Show Grid'
    TabOrder = 20
    OnClick = chkShowGridClick
  end
  object chkShowCoordinates: TCheckBox
    Left = 24
    Top = 351
    Width = 129
    Height = 17
    Caption = 'Show Coordinates'
    TabOrder = 21
    OnClick = chkShowCoordinatesClick
  end
  object SaveDialog: TSaveDialog
    Left = 832
    Top = 48
  end
end
