...
begin
  ...
  try
    ...
    FRouteMap.SetRoute(FRoute);// Store the route
    TotalDistance := 0;
    RouteText := 'Optimal Route:' + sLineBreak + sLineBreak;

    // Calculate total distance and build route text
    for i := 0 to High(FRoute) - 1 do
    begin
      Location := FRouteMap.GetLocation(FRoute[i]);
      RouteText := RouteText + Format('%d. %s' + sLineBreak, [i + 1, Location.Name]);

    end;

    // Add final destination
    Location := FRouteMap.GetLocation(FRoute[High(FRoute)]);
    RouteText := RouteText + Format('%d. %s' + sLineBreak + sLineBreak, [Length(FRoute), Location.Name]);
